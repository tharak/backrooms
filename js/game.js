...
document.querySelector('#send-message').addEventListener('click',async()=>{ const text=textArea.value.trim(), name=nameInput.value.trim() || 'Unknown wanderer', color=colorInput.value; const send=document.querySelector('#send-message'); if(!text)return; if(!config.messageToken){setStatus('The message marker has not been configured yet.');return;} localStorage.setItem('backrooms-marker-name',name); localStorage.setItem('backrooms-marker-color',color); send.disabled=true; try { const record={version:1,position:{x:+player.x.toFixed(2),z:+player.y.toFixed(2)},text:`${text}\n— ${name}`,author:{name,color},createdAt:new Date().toISOString()}; const r=await fetch(`${API}/repos/${config.messageOwner}/${config.messageRepo}/issues/${config.messageIssueNumber}/comments`,{method:'POST',headers:{Accept:'application/vnd.github+json',Authorization:`Bearer ${config.messageToken}`,'X-GitHub-Api-Version':config.githubApiVersion,'Content-Type':'application/json'},body:JSON.stringify({body:JSON.stringify(record)})}); const response=await r.json(); if(!r.ok)throw new Error(response.message||r.status); marks.push({...record,id:response.id}); textArea.value=''; document.querySelector('#count').textContent='0'; closePanels(); setStatus('The wall remembers what you wrote.'); }catch(err){setStatus(`The marker failed: ${err.message}.`);}finally{send.disabled=false;} });
