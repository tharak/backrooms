...
const messageEcho = document.querySelector('#message-echo');
const echoText = document.querySelector('#echo-text');
...
function revealNearbyMessage() { let nearest = null, distance = Infinity; for (const message of marks) { const d = Math.hypot(message.position.x - player.x, message.position.z - player.y); if (d < distance) { nearest = message; distance = d; } } if (nearest && distance < 2.1) { echoText.textContent = nearest.text; messageEcho.classList.remove('hidden'); } else messageEcho.classList.add('hidden'); }
