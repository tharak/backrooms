...
function markSurface() { for(let distance=.35;distance<2.5;distance+=.04) { const x=player.x+Math.cos(player.a)*distance, z=player.y+Math.sin(player.a)*distance; if(isWall(x,z)) return {x:+(x-Math.cos(player.a)*.035).toFixed(2),z:+(z-Math.sin(player.a)*.035).toFixed(2),surfaceAngle:+player.a.toFixed(3)}; } return {x:+(player.x+Math.cos(player.a)*.8).toFixed(2),z:+(player.y+Math.sin(player.a)*.8).toFixed(2),surfaceAngle:+player.a.toFixed(3)}; }
...
function draw() {
  ...
  for (const m of marks) { const dx=m.position.x-player.x, dy=m.position.z-player.y, dist=Math.hypot(dx,dy); let delta=Math.atan2(dy,dx)-player.a; delta=Math.atan2(Math.sin(delta),Math.cos(delta)); const surfaceAngle=Number.isFinite(m.surfaceAngle)?m.surfaceAngle:Math.atan2(-dy,-dx); let face=Math.atan2(player.y-m.position.z,player.x-m.position.x)-surfaceAngle; face=Math.atan2(Math.sin(face),Math.cos(face)); const width=Math.cos(face); if(dist<6 && Math.abs(delta)<fov/2 && width>.12) { const sx=w/2+(delta/(fov/2))*w/2, size=Math.max(10,Math.min(20,38/dist)), maxWidth=Math.min(260,w*.48)/width; ctx.font=`${size}px monospace`; const lines=[]; for(const paragraph of m.text.split('\n')) { let line=''; for(const word of paragraph.split(' ')) { const next=line?`${line} ${word}`:word; if(ctx.measureText(next).width>maxWidth&&line){lines.push(line);line=word;}else line=next; } if(line)lines.push(line); } const visible=lines.slice(0,5); ctx.save(); ctx.translate(sx,h/2); ctx.scale(width,1); ctx.textAlign='center'; ctx.lineWidth=3; ctx.strokeStyle='#29210f'; ctx.fillStyle=m.author?.color || '#e7df61'; visible.forEach((line,index)=>{const y=(index-(visible.length-1)/2)*size*1.3;ctx.strokeText(line,0,y);ctx.fillText(line,0,y);}); ctx.restore(); } }
}
...
document.querySelector('#send-message').addEventListener('click',async()=>{ const text=textArea.value.trim(), name=nameInput.value.trim() || 'Unknown wanderer', color=colorInput.value, token=tokenInput.value.trim(); const send=document.querySelector('#send-message'); if(!text)return; if(!token){setStatus('Paste your GitHub token to use the marker.');return;} localStorage.setItem('backrooms-marker-name',name); localStorage.setItem('backrooms-marker-color',color); sessionStorage.setItem('backrooms-player-token',token); send.disabled=true; try { const surface=markSurface(), record={version:1,position:{x:surface.x,z:surface.z},surfaceAngle:surface.surfaceAngle,text:`${text}\n— ${name}`,author:{name,color},createdAt:new Date().toISOString()}; const r=await fetch(`${API}/repos/${config.messageOwner}/${config.messageRepo}/issues/${config.messageIssueNumber}/comments`,{method:'POST',headers:{Accept:'application/vnd.github+json',Authorization:`Bearer ${token}`,'X-GitHub-Api-Version':config.githubApiVersion,'Content-Type':'application/json'},body:JSON.stringify({body:JSON.stringify(record)})}); const response=await r.json(); if(!r.ok)throw new Error(response.message||r.status); marks.push({...record,id:response.id}); textArea.value=''; document.querySelector('#count').textContent='0'; closePanels(); setStatus('The wall remembers what you wrote.'); }catch(err){setStatus(`The marker failed: ${err.message}.`);}finally{send.disabled=false;} });
