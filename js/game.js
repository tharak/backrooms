...
const wallNoteText = document.querySelector('#wall-note-text');
...
function showNearbyWallNote() { let nearest = null, distance = Infinity; for (const message of marks) { const d = Math.hypot(message.position.x - player.x, message.position.z - player.y); if (d < distance) { nearest = message; distance = d; } } if (nearest && distance < 2.25) { wallNoteText.textContent = nearest.text; wallNote.classList.remove('hidden'); } else wallNote.classList.add('hidden'); }
...
function draw() {
  ...
  for (const m of marks) { const dx=m.position.x-player.x, dy=m.position.z-player.y, dist=Math.hypot(dx,dy); let delta=Math.atan2(dy,dx)-player.a; delta=Math.atan2(Math.sin(delta),Math.cos(delta)); if(dist<6 && Math.abs(delta)<fov/2) { const sx=w/2+(delta/(fov/2))*w/2, size=Math.max(10,Math.min(20,38/dist)), maxWidth=Math.min(260,w*.48); ctx.font=`${size}px monospace`; const lines=[]; for(const paragraph of m.text.split('\n')) { let line=''; for(const word of paragraph.split(' ')) { const next=line?`${line} ${word}`:word; if(ctx.measureText(next).width>maxWidth&&line){lines.push(line);line=word;}else line=next; } if(line)lines.push(line); } const visible=lines.slice(0,5); ctx.textAlign='center'; ctx.lineWidth=3; ctx.strokeStyle='#29210f'; ctx.fillStyle=m.author?.color || '#e7df61'; visible.forEach((line,index)=>{const y=h/2+(index-(visible.length-1)/2)*size*1.3;ctx.strokeText(line,sx,y);ctx.fillText(line,sx,y);}); ctx.textAlign='start'; } }
}
function update(now) { const dt=Math.min(.05,(now-previous)/1000); previous=now; if(started){ let nx=player.x, ny=player.y; const forward=(keys.has('w')?1:0)-(keys.has('s')?1:0), side=(keys.has('d')?1:0)-(keys.has('a')?1:0); const speed=player.speed*dt; nx+=Math.cos(player.a)*forward*speed+Math.cos(player.a+Math.PI/2)*side*speed; ny+=Math.sin(player.a)*forward*speed+Math.sin(player.a+Math.PI/2)*side*speed; if(!isWall(nx,player.y))player.x=nx; if(!isWall(player.x,ny))player.y=ny; showNearbyWallNote(); } draw(); requestAnimationFrame(update); }
