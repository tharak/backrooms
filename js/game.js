...
async function beginLogin(){ if(!config.oauthBrokerUrl){ loginStatus.textContent='This marker has not been configured with its GitHub ink broker yet.'; return; } const button=document.querySelector('#start-login'); const broker=config.oauthBrokerUrl.replace(/\/$/,''); button.disabled=true; loginStatus.textContent='Asking GitHub for ink…'; try { const r=await fetch(`${broker}/device`,{method:'POST'}); const data=await r.json(); if(!r.ok)throw new Error(data.error||r.status); deviceCode.textContent=data.user_code; deviceCode.classList.remove('hidden'); verifyLink.href=data.verification_uri; verifyLink.classList.remove('hidden'); verifyLink.click(); loginStatus.textContent='Enter the code on GitHub. Waiting for ink…'; const until=Date.now()+data.expires_in*1000; let interval=(data.interval||5)*1000; while(Date.now()<until){ await new Promise(resolve=>setTimeout(resolve,interval)); const poll=await fetch(`${broker}/token`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({device_code:data.device_code})}); const answer=await poll.json(); if(answer.access_token){ token=answer.access_token; sessionStorage.setItem('backrooms-github-token',token); markerState(); loginStatus.textContent='The marker is full.'; setTimeout(closePanels,900); return; } if(answer.error==='slow_down')interval+=5000; if(answer.error && answer.error!=='authorization_pending')throw new Error(answer.error); } throw new Error('expired_token'); } catch(err) { loginStatus.textContent=`GitHub ink failed: ${err.message}. Try again.`; } finally {button.disabled=false;} }
